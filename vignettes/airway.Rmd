...
*SummarizedExperiment* object of the *airway* experiment data
...
geofile <- paste0(dir, "GSE52778_series_matrix.txt")
...
```{r}
srafile <- paste0(dir, "SraRunInfo_SRP033351.csv")
...
The argument `--split-3` was necessary for run SRR1039515, in order to
obtain two files for the two paired-end reads which had equal number
of reads.

# Aligning reads

The reads were aligned to the Ensembl release 75 human reference
genome using the
...
fls <- paste0("aligned/",rownames(sampleTable), ".bam")
library( "Rsamtools" )
bamLst <- BamFileList( fls, yieldSize=2000000 )
```

The following `summarizeOverlaps` call distributed the 8 paired-end
BAM files to 8 workers. This used a maximum of 10 Gb per worker and
the time elapsed was 22 minutes.
